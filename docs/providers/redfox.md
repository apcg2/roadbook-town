...
帖子正文是主要体验证据，两条正面短评均来自正文；评论区只补充排队、停车、拥挤等限制信息。评论无权限、额度不足、接口异常或超时时，工具自动进入 `post-only`，记录 `permission-or-quota`、`provider-error`或`timeout`，并继续使用正文，不保持pending。口碑覆盖至少2位不同作者（目标3位），每条都记录 `sourceType`。只有正文去广告后仍不足时，才使用 `insufficient`。不得复制长文、用户名或个人资料，也不得编造评价。
