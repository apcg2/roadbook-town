...
function reviews(r){
  if(r.status==='insufficient') return '<div class="spot-reviews"><h4>小红书网友</h4><p class="review-line">样本不足</p></div>';
  return `<div class="spot-reviews"><h4>小红书网友</h4><div class="review-line">${['good','mixed'].map(kind=>`<span class="review-group ${kind}">${art(`review-${kind}`,'','0 0 18 18')}${r[kind].map(v=>`<span>${esc(v.text)}</span>`).join('<span class="review-separator" aria-hidden="true">·</span>')}</span>`).join('')}</div></div>`;
}
